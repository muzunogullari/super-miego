...
The game has 5 levels. All 5 are currently generated procedurally by `LevelGenerator.swift` via `LevelManager` (defined in `Level1.swift`, which now acts as a wrapper/entry-point file rather than a hand-authored layout).
...
| `=` | Platform (one-way) | BlockNode (.platform) | platform |
| `[` | Pipe top-left | SKSpriteNode (via `createPipe`) | ground |
| `P` | Pipe body-left | SKSpriteNode (via `createPipe`) | ground |
| `\|` | Pipe body-right | SKSpriteNode (via `createPipe`) | ground |
...
3. Wire it into `LevelManager.getData(for:)` (currently in `Level1.swift`) or extend `LevelGenerator` if you want the level to stay inside the procedural progression flow.
